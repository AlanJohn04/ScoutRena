...
export default function MarketTradingFloor() {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-indigo-300 font-semibold uppercase tracking-widest text-sm">Loading Market Data...</div>;
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-[#ff2020]/30 pb-8 gap-6 z-10">
        <div>
          <h2 className="text-3xl font-black text-white tracking-widest flex items-center gap-3 uppercase glitch-hover">
            <Activity className="text-[#ff2020] w-6 h-6 animate-pulse" />
            [ LIVE_MARKET_TERMINAL ]
          </h2>
          <p className="text-xs text-[#888888] font-bold mt-1 tracking-widest uppercase">SYS_LOG: Real-time supply/demand metrics and candidate valuations</p>
        </div>
        <div className="flex items-center gap-8 border border-[#ff2020]/30 py-3 px-6 bg-black">
          <div className="text-right">
            <span className="text-[10px] text-[#ff2020] uppercase font-bold tracking-widest block mb-1">[TOTAL_CAP]</span>
          ...
          <div className="text-right">
            <span className="text-[10px] text-[#ff2020] uppercase font-bold tracking-widest block mb-1">[AVG_POTENTIAL]</span>
            <p className="text-2xl font-bold text-white tracking-tight">
      ...
      >
        {mockMarketIndices.map((idx, index) => (
          <motion.div variants={listItemVariants} key={idx.id} className={`p-6 flex flex-col justify-between hover:bg-[#ff2020]/10 transition-colors cursor-crosshair border-b lg:border-b-0 ${index !== mockMarketIndices.length - 1 ? 'lg:border-r border-[#ff2020]/30' : ''}`}>
            <div>
              <span className="text-[10px] text-[#ff2020] font-bold uppercase tracking-widest block mb-2">[ {idx.name} ]</span>
              <p className="text-3xl font-bold text-white tracking-tight transition-colors">
                <AnimatedCounter value={idx.multiplier} suffix="x" />
              </p>
            </div>
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-[#ff2020]/30 text-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 z-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          >
            ...
            <h3 className="text-sm font-bold text-white tracking-widest mb-6 flex items-center gap-2 uppercase">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {risingStars.map((candidate) => (
                <div key={candidate.id} className="p-6 border border-[#ff2020]/30 bg-black hover:bg-[#ff2020]/10 transition-colors flex flex-col gap-5 relative group cursor-crosshair">
                  <div className="absolute top-4 right-4 text-[#ff2020] text-[10px] font-bold px-2 py-1 uppercase tracking-widest border border-[#ff2020]/50">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-white tracking-widest uppercase">{candidate.name}</h4>
                      <p className="text-[10px] text-[#ff2020] font-bold uppercase tracking-widest mt-1">[{candidate.role}]</p>
                    </div>
                  </div>
                  
                  <div className="flex justify-between items-center text-xs mt-2 pt-5 border-t border-[#ff2020]/30">
                    <div>
                      <span className="text-[#888888] block text-[9px] font-bold uppercase tracking-widest mb-1">Valuation</span>
                      <span className="font-bold text-white text-sm">{candidate.currentValue} TT</span>
                    </div>
                    <div className="text-right">
                      <span className="text-[#888888] block text-[9px] font-bold uppercase tracking-widest mb-1">Score</span>
                      <span className="font-bold text-white text-sm">{candidate.potentialScore}/100</span>
                    </div>
                  </div>

                  <Link 
                    href={`/student/${candidate.id}`}
                    className="w-full text-center py-2.5 bg-transparent border border-white hover:bg-white hover:text-black text-xs font-bold text-white uppercase tracking-widest transition-colors mt-2"
                  >
                    [ VIEW_DOSSIER ]
                  </Link>
                </div>
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#ff2020]/30 text-[10px] text-[#ff2020] uppercase font-bold tracking-widest">
                  ...
                  {candidates.map((c) => (
                    <motion.tr variants={listItemVariants} key={c.id} className="hover:bg-[#ff2020]/10 transition-colors group cursor-crosshair">
                      <td className="py-4 pl-2 flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-white tracking-widest uppercase">{c.name}</p>
                          <span className="text-[10px] text-[#888888] font-bold uppercase tracking-widest">[{c.college}]</span>
                        </div>
                      </td>
                      <td className="py-4 text-xs font-bold text-white uppercase tracking-widest">{c.role}</td>
                      <td className="py-4 text-sm font-bold">
                        <span className="text-white">{c.potentialScore}</span>
                        <span className="text-[#888888] text-[10px]">/100</span>
                      </td>
                      <td className="py-4 text-right text-sm font-bold text-white">
                        {c.currentValue.toLocaleString()} TT
                      </td>
                      <td className="py-4 text-right pr-2">
                        <span className="inline-block text-[10px] text-[#ff2020] py-1 px-2 font-bold tracking-widest uppercase border border-[#ff2020]/30">
        ...
        >
            ...
            >
              {mockTransactions.map((tx, idx) => (
                <motion.div variants={listItemVariants} key={idx} className={`p-4 bg-black flex flex-col gap-3 relative hover:bg-[#ff2020]/10 transition-colors border-b border-[#ff2020]/30 ${idx === mockTransactions.length - 1 ? 'border-b-0' : ''}`}>
                  ...
                  <p className="text-xs text-white font-medium leading-relaxed">{tx.details}</p>
                  <div className="flex justify-between items-center text-[9px] text-[#888888] font-bold tracking-widest uppercase mt-2 pt-3 border-t border-[#ff2020]/30">
          ...
          <div className="mt-8 pt-6 border-t border-[#ff2020]/30 flex flex-col gap-3 text-center text-[10px] font-bold tracking-widest uppercase">
