...
export default function StudentDashboard() {
  ...
  if (!profileData || !user) {
    return <div className="min-h-screen flex items-center justify-center text-indigo-300 text-sm font-semibold tracking-widest uppercase">Loading Talent DNA...</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-10 min-h-screen text-white relative overflow-hidden p-6 md:p-10">
      {/* Decorative Orbs */}
      <div className="absolute top-[0%] right-[10%] w-[40%] h-[40%] bg-indigo-600/10 rounded-full blur-[120px] pointer-events-none -z-10"></div>
      <div className="absolute bottom-[0%] left-[10%] w-[40%] h-[40%] bg-cyan-600/10 rounded-full blur-[120px] pointer-events-none -z-10"></div>

      {/* Profile Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/10 pb-6 gap-6 z-10">
        <div className="flex items-center gap-6">
          <img src={profileData.avatar} alt={profileData.name} className="w-24 h-24 rounded-full object-cover border-2 border-indigo-500/30 shadow-[0_0_20px_rgba(99,102,241,0.2)]" />
          <div>
        ...
        <div className="flex items-center gap-8 glass-panel p-5">
          <div className="text-right">
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold block mb-1">Personal Valuation</span>
            <p className="text-3xl font-bold text-white mono-font">{profileData.currentValue.toLocaleString()} TT</p>
          </div>
          <div className="w-px h-12 bg-white/10"></div>
          <div className="text-right">
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold block mb-1">Weekly Growth</span>
            <p className="text-3xl font-bold text-emerald-400 flex items-center justify-end mono-font">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 z-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="glass-panel p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {profileData.badges.map((badge: any, idx: number) => (
                <div 
                  key={idx}
                  className="p-5 bg-slate-900/60 border border-white/5 rounded-xl text-center flex flex-col items-center gap-3 hover:-translate-y-1 transition-transform group shadow-lg"
                ...
                >
                  ...
                  <span className="text-[9px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 uppercase tracking-widest mt-2 rounded-full">
                    Verified
                  </span>
                </div>
              ))}
          ...
          <div className="glass-panel p-8 relative overflow-hidden border-indigo-500/30 group">
            <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-indigo-600/10 rounded-full blur-3xl group-hover:bg-indigo-600/20 transition-colors"></div>
            ...
            <p className="text-sm text-slate-400 mb-6 font-medium leading-relaxed max-w-2xl">
            ...
            {parseSuccess && (
              <div className="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm font-bold flex items-center justify-center gap-2">
            ...
            <textarea
              ...
              className="w-full bg-slate-900/50 border border-slate-700/50 p-4 rounded-xl text-sm font-medium text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all resize-none mb-6 shadow-inner"
            />
            
            <button
              onClick={handleResumeParse}
              disabled={parsing || !resumeText.trim()}
              className="glass-button w-full py-4 text-sm font-bold text-white disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-widest"
            >
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="glass-panel p-8 flex flex-col items-center">
            ...
            <div className="grid grid-cols-2 gap-4 w-full mt-6 border-t border-white/10 pt-6">
              <div className="text-center bg-slate-900/50 rounded-xl p-4 border border-white/5 shadow-inner">
                <span className="text-slate-500 block text-[10px] font-bold tracking-widest uppercase mb-1">Future Potential</span>
              ...
              <div className="text-center bg-slate-900/50 rounded-xl p-4 border border-white/5 shadow-inner">
                <span className="text-slate-500 block text-[10px] font-bold tracking-widest uppercase mb-1">Velocity</span>
          ...
          <div className="glass-panel p-8">
            <h3 className="text-lg font-bold text-white tracking-wide mb-4 flex items-center gap-2">
              <Lightbulb className="text-yellow-400 w-5 h-5 drop-shadow-[0_0_5px_rgba(250,204,21,0.5)]" /> AI Advisor Insights
            </h3>
            <p className="text-sm text-slate-300 font-medium leading-relaxed bg-slate-900/50 rounded-xl border border-white/5 p-5 shadow-inner">
              ...
              {Object.entries(profileData.compatibility).map(([companyName, percentage]) => (
                <div key={companyName} className="flex flex-col gap-2">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="font-bold mono-font text-indigo-400 text-xs">{percentage as number}%</span>
                  </div>
